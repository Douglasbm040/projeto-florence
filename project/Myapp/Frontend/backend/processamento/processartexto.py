...
from ....Frontend.screen  import Myapp
...
base=[('LPP','AGE'),
      ...
      ('vasculares','Petrolatum'),
      ('queimaduras 1','Petrolatum'),
      ('queimaduras 2','Petrolatum'),
      ('UPP','Petrolatum'),
      ('LPP','Petrolatum'),
      ('enxerto','Petrolatum'),
...
y_train.drop(delete,inplace=True)
y_train=y_train.drop(columns=produtos,inplace=True)
y_train=y.astype('int')
y_train=y.astype('int')
modelo = MLPClassifier (verbose = True, hidden_layer_sizes=(27,19), activation="relu",max_iter = 10000) 
modelo.fit(x_train, y_train)


previsoes = modelo.predict(k)

rede_neural=df['material'].drop(delete).tolist()
rede_neural=rede_neural[previsoes[0]]
